...
{
    XMFLOAT4 EmitterPositionTime;
    XMFLOAT4 GravityDeltaTime;
    XMFLOAT4 EmitterParams;
};

struct PostProcessConstants
{
    XMFLOAT4 ScreenSizeTime;
    XMFLOAT4 EffectFlags;
...
private:
    struct RenderMaterial
    {
        ...
        UINT RoughnessTextureIndex = 3;
        UINT MetallicTextureIndex = 4;
    };
    ...
    void UploadPostProcessConstants();
    void UploadParticleConstants();
    void UploadParticleSimConstants(float totalTime, float deltaTime);
    ...
    void UpdatePostProcessControls(const InputDevice& input);
    void UpdateParticleControls(const InputDevice& input, float dt);
    ...
    void SimulateParticles(ID3D12GraphicsCommandList* cmdList);
...
private:
    ...
    ComPtr<ID3D12Resource> mParticleConstantBuffer;
    ComPtr<ID3D12Resource> mParticleSimConstantBuffer;
    ComPtr<ID3D12Resource> mPostProcessConstantBuffer;
    ...
    ComPtr<ID3D12Resource> mIrradianceMapUpload;
    ComPtr<ID3D12Resource> mPrefilteredEnvMapUpload;
    ComPtr<ID3D12Resource> mBrdfIntegrationMapUpload;
    ...
    ComPtr<ID3D12Resource> mParticleBuffers[2];
    ComPtr<ID3D12Resource> mParticleBufferUploads[2];
    ComPtr<ID3D12Resource> mParticleCounterBuffers[2];
    ComPtr<ID3D12Resource> mParticleCounterResetUpload;
    D3D12_RESOURCE_STATES mParticleBufferStates[2] = {};
    D3D12_RESOURCE_STATES mParticleCounterStates[2] = {};
    ...
    ComPtr<ID3D12RootSignature> mParticleRenderRootSignature;
    ...
    ComPtr<ID3D12PipelineState> mParticleRenderPSO;
    ...
    ParticleSimConstants mParticleSimConstants = {};
    ...
    float mYaw = 0.0f;
    float mPitch = -0.08f;
    UINT mParticleReadBuffer = 0;
    UINT mParticleWriteBuffer = 1;
    static constexpr UINT ParticleCount = 4096u;
    XMFLOAT3 mParticleEmitterPos = { 0.0f, 22.0f, -38.0f };
    float mParticleSpawnChance = 0.035f;
    float mParticleLifetime = 2.8f;
    ...
    bool mChromaticAberrationEnabled = true;
    bool mFilmGrainEnabled = true;
};
