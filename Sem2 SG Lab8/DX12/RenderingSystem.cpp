...
namespace
{
    ...
    {
        return ClampFloat(sqrtf(2.0f / ((std::max)(shininess, 1.0f) + 2.0f)), 0.04f, 1.0f);
    }
    ...
    std::wstring FindExistingModelPath()
    {
        ...
        {
            L"Models/PBR/Cerberus_by_Andrew_Maximov/Cerberus_LP.FBX",
            L"DX12/Models/PBR/Cerberus_by_Andrew_Maximov/Cerberus_LP.FBX",
            L"x64/Debug/Models/PBR/Cerberus_by_Andrew_Maximov/Cerberus_LP.FBX",
            L"Models/sponza.obj",
    ...
    std::wstring FindExistingAssetPath(std::initializer_list<std::wstring> candidates)
    {
        for (const auto& candidate : candidates)
        {
            if (FileExists(candidate))
    ...
    void AppendMeshData(ObjMeshData& dst, const ObjMeshData& src, const XMFLOAT3& offset)
    {
        const uint32_t vertexBase = (uint32_t)dst.Vertices.size();
        const uint32_t indexBase = (uint32_t)dst.Indices.size();
        const size_t materialBase = dst.Materials.size();

        dst.Materials.insert(dst.Materials.end(), src.Materials.begin(), src.Materials.end());
        for (size_t i = materialBase; i < dst.Materials.size(); ++i)
        {
            auto& material = dst.Materials[i];
        ...
        for (VertexPosNormal vertex : src.Vertices)
        {
        ...
        for (uint32_t index : src.Indices)
            dst.Indices.push_back(vertexBase + index);

        for (ObjSubset subset : src.Subsets)
        {
            subset.IndexStart += indexBase;
            for (size_t sourceIndex = 0; sourceIndex < src.Materials.size(); ++sourceIndex)
            {
                if (src.Materials[sourceIndex].Name == subset.MaterialName)
                {
                    subset.MaterialName = dst.Materials[materialBase + sourceIndex].Name;
                    break;
                }
            }
    ...
    std::wstring FindExistingIblPath(const std::wstring& filename)
    {
        const std::wstring candidates[] =
        {
            L"IBL/" + filename,
    ...
    {
        return format == DXGI_FORMAT_BC6H_UF16 || format == DXGI_FORMAT_BC6H_SF16;
    }
    ...
    bool ComputeSurfaceInfo(UINT width, UINT height, DXGI_FORMAT format, UINT64& rowPitch, UINT64& slicePitch)
    {
        if (IsBlockCompressed(format))
        {
            const UINT64 blocksWide = (std::max)(1u, (width + 3u) / 4u);
            const UINT64 blocksHigh = (std::max)(1u, (height + 3u) / 4u);
            rowPitch = blocksWide * 16u;
        ...
        rowPitch = ((UINT64)width * bpp + 7u) / 8u;
        slicePitch = rowPitch * height;
        return true;
    }

    bool LoadDdsTextureFromFile(ID3D12Device* device,
...
#pragma pack(pop)
        ...
        bool ok = fread(&magic, sizeof(magic), 1, file) == 1
            ...
            && header.Format.FourCC == 0x30315844;

        if (ok)
            ok = fread(&header10, sizeof(header10), 1, file) == 1;
        ...
        const DXGI_FORMAT format = (DXGI_FORMAT)header10.DxgiFormat;
        if (format != DXGI_FORMAT_R32G32_FLOAT && format != DXGI_FORMAT_BC6H_UF16)
        {
            fclose(file);
            return false;
        }
        ...
        const UINT arraySize = (std::max)(1u, header10.ArraySize) * (isCube ? 6u : 1u);
        const UINT subresourceCount = mipCount * arraySize;

        std::vector<uint8_t> pixels;
        fseek(file, 0, SEEK_END);
        const long fileSize = ftell(file);
        const long pixelOffset = 4 + sizeof(DdsHeader) + sizeof(DdsHeaderDx10);
        if (fileSize <= pixelOffset)
        {
            fclose(file);
            return false;
        }

        pixels.resize((size_t)(fileSize - pixelOffset));
        fseek(file, pixelOffset, SEEK_SET);
        ok = fread(pixels.data(), 1, pixels.size(), file) == pixels.size();
        fclose(file);
        ...
        if (FAILED(device->CreateCommittedResource(&defaultHeapProps, D3D12_HEAP_FLAG_NONE, &texDesc,
        ...
        for (UINT item = 0; item < arraySize; ++item)
        {
            ...
            for (UINT mip = 0; mip < mipCount; ++mip)
            {
                UINT64 rowPitch = 0;
                UINT64 slicePitch = 0;
                if (!ComputeSurfaceInfo(width, height, format, rowPitch, slicePitch)
                    || offset + (size_t)slicePitch > pixels.size())
                    return false;

                D3D12_SUBRESOURCE_DATA& subresource = subresources[item * mipCount + mip];
                subresource.pData = pixels.data() + offset;
                subresource.RowPitch = (LONG_PTR)rowPitch;
                subresource.SlicePitch = (LONG_PTR)slicePitch;
                offset += (size_t)slicePitch;
                width = (std::max)(1u, width >> 1);
                height = (std::max)(1u, height >> 1);
            }
        }

        UpdateSubresources(cmdList, texture.Get(), upload.Get(), 0, 0, subresourceCount, subresources.data());
        auto barrier = CD3DX12_RESOURCE_BARRIER::Transition(
            texture.Get(), D3D12_RESOURCE_STATE_COPY_DEST,
            D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE | D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE);
        cmdList->ResourceBarrier(1, &barrier);

        srvDimension = isCube ? D3D12_SRV_DIMENSION_TEXTURECUBE : D3D12_SRV_DIMENSION_TEXTURE2D;
        return true;
    }
...
void RenderingSystem::BuildModelGeometry()
{
    ...
    const std::wstring cerberusPath = FindExistingAssetPath({
        L"Models/PBR/Cerberus_by_Andrew_Maximov/Cerberus_LP.FBX",
        L"DX12/Models/PBR/Cerberus_by_Andrew_Maximov/Cerberus_LP.FBX",
        L"x64/Debug/Models/PBR/Cerberus_by_Andrew_Maximov/Cerberus_LP.FBX"
        });
    const std::wstring woodRootPath = FindExistingAssetPath({
        L"Models/PBR/wood_root/Aset_wood_root_M_rkswd_LOD0.fbx",
        L"DX12/Models/PBR/wood_root/Aset_wood_root_M_rkswd_LOD0.fbx",
        L"x64/Debug/Models/PBR/wood_root/Aset_wood_root_M_rkswd_LOD0.fbx"
        });

    ObjMeshData pbrMesh;
    if (!cerberusPath.empty() && ObjLoader::LoadObjPosNormal(cerberusPath, pbrMesh, true))
        AppendMeshData(mMeshData, pbrMesh, XMFLOAT3(-55.0f, 0.0f, 0.0f));

    pbrMesh = {};
    if (!woodRootPath.empty() && ObjLoader::LoadObjPosNormal(woodRootPath, pbrMesh, true))
        AppendMeshData(mMeshData, pbrMesh, XMFLOAT3(55.0f, 0.0f, 0.0f));
...
void RenderingSystem::CreateFallbackTexture()
{
    ...
    uint32_t roughnessPixels[width * height] = {};
    uint32_t metallicPixels[width * height] = {};
    for (UINT i = 0; i < width * height; ++i)
    {
        normalPixels[i] = 0xffff8080u;
        displacementPixels[i] = 0xff808080u;
        roughnessPixels[i] = 0xff999999u;
        metallicPixels[i] = 0xff000000u;
    }

    createTexture(diffusePixels);
    createTexture(normalPixels);
    createTexture(displacementPixels);
    createTexture(roughnessPixels);
    createTexture(metallicPixels);
}
...
void RenderingSystem::BuildTextureResources()
{
    ...
    for (UINT i = 0; i < (UINT)mMeshData.Materials.size(); ++i)
    {
        ...
        dst.Roughness = ShininessToRoughness(src.Shininess);
        dst.Metallic = Luminance(src.Specular) > 0.65f && Luminance(src.Diffuse) < 0.45f ? 1.0f : 0.0f;
        dst.AmbientOcclusion = 1.0f;
        ...
        loadTextureIndex(src.RoughnessTexture, dst.RoughnessTextureIndex);
        loadTextureIndex(src.MetallicTexture, dst.MetallicTextureIndex);
    }
...
void RenderingSystem::BuildConstantBuffers()
{
    ...
    const UINT postProcessConstantByteSize = (sizeof(PostProcessConstants) + 255) & ~255;
    ...
    auto postDesc = CD3DX12_RESOURCE_DESC::Buffer(postProcessConstantByteSize);
    ...
    ThrowIfFailed(mDevice->CreateCommittedResource(&uploadHeapProps, D3D12_HEAP_FLAG_NONE, &postDesc,
        D3D12_RESOURCE_STATE_GENERIC_READ, nullptr, IID_PPV_ARGS(&mPostProcessConstantBuffer)));
}

void RenderingSystem::BuildIblResources()
{
    D3D12_SRV_DIMENSION dimension = D3D12_SRV_DIMENSION_UNKNOWN;
    const std::wstring irradiancePath = FindExistingIblPath(L"IrradianceMap_BC6U.dds");
    const std::wstring prefilteredPath = FindExistingIblPath(L"PreFilteredEnvMap_BC6U.dds");
    const std::wstring integrationPath = FindExistingIblPath(L"IntegrationMap.dds");

    if (irradiancePath.empty()
        || !LoadDdsTextureFromFile(mDevice, mCmdList, irradiancePath, mIrradianceMap, mIrradianceMapUpload, dimension))
        throw std::runtime_error("Failed to load IBL irradiance map");

    if (prefilteredPath.empty()
        || !LoadDdsTextureFromFile(mDevice, mCmdList, prefilteredPath, mPrefilteredEnvMap, mPrefilteredEnvMapUpload, dimension))
        throw std::runtime_error("Failed to load IBL prefiltered environment map");

    if (integrationPath.empty()
        || !LoadDdsTextureFromFile(mDevice, mCmdList, integrationPath, mBrdfIntegrationMap, mBrdfIntegrationMapUpload, dimension))
        throw std::runtime_error("Failed to load IBL BRDF integration map");
}
...
void RenderingSystem::BuildPostProcessResources()
{
    ...
    clearValue.Color[2] = 0.075f;
    clearValue.Color[3] = 1.0f;

    auto sceneDesc = CD3DX12_RESOURCE_DESC::Tex2D(
    ...
    ThrowIfFailed(mDevice->CreateDescriptorHeap(&rtvHeapDesc, IID_PPV_ARGS(&mSceneColorRtvHeap)));

    D3D12_RENDER_TARGET_VIEW_DESC rtvDesc = {};
    rtvDesc.ViewDimension = D3D12_RTV_DIMENSION_TEXTURE2D;
    rtvDesc.Format = DXGI_FORMAT_R16G16B16A16_FLOAT;
    rtvDesc.Texture2D.MipSlice = 0;
    mDevice->CreateRenderTargetView(mSceneColor.Get(), &rtvDesc, mSceneColorRtvHeap->GetCPUDescriptorHandleForHeapStart());
}

void RenderingSystem::BuildPostProcessDescriptors()
{
    if (!mSceneColor)
        return;
    ...
    srvDesc.ViewDimension = D3D12_SRV_DIMENSION_TEXTURE2D;
    srvDesc.Texture2D.MipLevels = 1;
    mDevice->CreateShaderResourceView(mSceneColor.Get(), &srvDesc, mPostProcessSrvHeap->GetCPUDescriptorHandleForHeapStart());
}
...
void RenderingSystem::BuildLightingDescriptors()
{
    ...
    D3D12_DESCRIPTOR_HEAP_DESC srvHeapDesc = {};
    srvHeapDesc.NumDescriptors = Gbuffer::TargetCount + 4;
    srvHeapDesc.Type = D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV;
    srvHeapDesc.Flags = D3D12_DESCRIPTOR_HEAP_FLAG_SHADER_VISIBLE;
    ...
    cubeSrvDesc.Format = DXGI_FORMAT_BC6H_UF16;
    cubeSrvDesc.ViewDimension = D3D12_SRV_DIMENSION_TEXTURECUBE;
    cubeSrvDesc.TextureCube.MostDetailedMip = 0;
    cubeSrvDesc.TextureCube.MipLevels = UINT_MAX;
    cubeSrvDesc.TextureCube.ResourceMinLODClamp = 0.0f;

    if (mIrradianceMap)
        mDevice->CreateShaderResourceView(mIrradianceMap.Get(), &cubeSrvDesc, dst);
    dst.Offset(1, mSrvDescriptorSize);

    if (mPrefilteredEnvMap)
        mDevice->CreateShaderResourceView(mPrefilteredEnvMap.Get(), &cubeSrvDesc, dst);
    dst.Offset(1, mSrvDescriptorSize);

    D3D12_SHADER_RESOURCE_VIEW_DESC brdfSrvDesc = {};
    brdfSrvDesc.Shader4ComponentMapping = D3D12_DEFAULT_SHADER_4_COMPONENT_MAPPING;
    brdfSrvDesc.Format = DXGI_FORMAT_R32G32_FLOAT;
    ...
    if (mBrdfIntegrationMap)
        mDevice->CreateShaderResourceView(mBrdfIntegrationMap.Get(), &brdfSrvDesc, dst);
}
...
{
    D3D12_DESCRIPTOR_RANGE texTables[5] = {};
    ...
    rootParams[4].DescriptorTable.pDescriptorRanges = &texTables[3];
    rootParams[4].ShaderVisibility = D3D12_SHADER_VISIBILITY_PIXEL;
    rootParams[5].ParameterType = D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE;
    rootParams[5].DescriptorTable.NumDescriptorRanges = 1;
    rootParams[5].DescriptorTable.pDescriptorRanges = &texTables[4];
...
void RenderingSystem::BuildLightingRootSignature()
{
    ...
    D3D12_DESCRIPTOR_RANGE iblTable = {};
    iblTable.RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_SRV;
    iblTable.NumDescriptors = 3;
    iblTable.BaseShaderRegister = 4;
    iblTable.OffsetInDescriptorsFromTableStart = D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND;
    ...
    rootParams[3].ParameterType = D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE;
    rootParams[3].DescriptorTable.NumDescriptorRanges = 1;
    rootParams[3].DescriptorTable.pDescriptorRanges = &iblTable;
...
{
    D3D12_DESCRIPTOR_RANGE sceneColorTable = {};
    sceneColorTable.RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_SRV;
    sceneColorTable.NumDescriptors = 1;
    sceneColorTable.BaseShaderRegister = 0;
    sceneColorTable.OffsetInDescriptorsFromTableStart = D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND;
    ...
    rootParams[1].ParameterType = D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE;
    rootParams[1].DescriptorTable.NumDescriptorRanges = 1;
    rootParams[1].DescriptorTable.pDescriptorRanges = &sceneColorTable;
    rootParams[1].ShaderVisibility = D3D12_SHADER_VISIBILITY_PIXEL;
...
void RenderingSystem::BuildPipelineStates()
{
    ...
    ComPtr<ID3DBlob> lVs;
    ComPtr<ID3DBlob> lPs;
    ComPtr<ID3DBlob> ppVs;
    ComPtr<ID3DBlob> ppPs;
    ComPtr<ID3DBlob> sVs;
    ComPtr<ID3DBlob> pCs;
    ComPtr<ID3DBlob> pVs;
    ComPtr<ID3DBlob> pGs;
    ...
    CompileShader(L"Shaders/DeferredLightingVS.hlsl", "VSMain", "vs_5_0", ppVs);
    CompileShader(L"Shaders/PostProcessPS.hlsl", "PSMain", "ps_5_0", ppPs);
    ...
    postDesc.VS = { ppVs->GetBufferPointer(), ppVs->GetBufferSize() };
    postDesc.PS = { ppPs->GetBufferPointer(), ppPs->GetBufferSize() };
    CD3DX12_RASTERIZER_DESC postRasterizer(D3D12_DEFAULT);
    postRasterizer.CullMode = D3D12_CULL_MODE_NONE;
    postDesc.RasterizerState = postRasterizer;
    ...
    ThrowIfFailed(mDevice->CreateGraphicsPipelineState(&postDesc, IID_PPV_ARGS(&mPostProcessPSO)));
...
void RenderingSystem::UpdatePostProcessControls(const InputDevice& input)
{
    ...
    if (input.WasKeyPressed(VK_F7))
        mChromaticAberrationEnabled = !mChromaticAberrationEnabled;
    if (input.WasKeyPressed(VK_F8))
        mFilmGrainEnabled = !mFilmGrainEnabled;
}
...
void RenderingSystem::Update(float totalTime, float deltaTime, const InputDevice& input)
{
    ...
    UpdatePostProcessControls(input);
    ...
    mPostProcessConstants.ScreenSizeTime = XMFLOAT4((float)mRenderWidth, (float)mRenderHeight, totalTime, 0.0f);
    mPostProcessConstants.EffectFlags = XMFLOAT4(
        mGammaCorrectionEnabled ? 1.0f : 0.0f,
        mDitheringEnabled ? 1.0f : 0.0f,
        mVignetteEnabled ? 1.0f : 0.0f,
        mChromaticAberrationEnabled ? 1.0f : 0.0f);
    mPostProcessConstants.EffectParams = XMFLOAT4(
        mPostProcessEnabled ? 1.0f : 0.0f,
        mFilmGrainEnabled ? 1.0f : 0.0f,
        1.0f,
        3.0f);
...
std::wstring RenderingSystem::StatusText() const
{
    ...
    text << L"CG lab8 | PBR Cook-Torrance | HDR post FX | CSM+PCF | GPU particles " << ParticleCount
        ...
        << L" | F3 post " << (mPostProcessEnabled ? L"ON" : L"OFF")
        << L" | F4 gamma " << (mGammaCorrectionEnabled ? L"ON" : L"OFF")
        << L" | F5 dither " << (mDitheringEnabled ? L"ON" : L"OFF")
        << L" | F6 vignette " << (mVignetteEnabled ? L"ON" : L"OFF")
        << L" | F7 chroma " << (mChromaticAberrationEnabled ? L"ON" : L"OFF")
        << L" | F8 grain " << (mFilmGrainEnabled ? L"ON" : L"OFF");
...
void RenderingSystem::UploadPostProcessConstants()
{
    void* mapped = nullptr;
    ThrowIfFailed(mPostProcessConstantBuffer->Map(0, nullptr, &mapped));
    memcpy(mapped, &mPostProcessConstants, sizeof(PostProcessConstants));
    mPostProcessConstantBuffer->Unmap(0, nullptr);
}
    ...
    D3D12_CPU_DESCRIPTOR_HANDLE depthStencilView)
{
    ...
    for (UINT i = 0; i < (UINT)mRenderSubsets.size(); ++i)
    {
        ...
        CD3DX12_GPU_DESCRIPTOR_HANDLE roughnessTextureHandle(mTextureHeap->GetGPUDescriptorHandleForHeapStart());
        roughnessTextureHandle.Offset(material.RoughnessTextureIndex, mSrvDescriptorSize);
        CD3DX12_GPU_DESCRIPTOR_HANDLE metallicTextureHandle(mTextureHeap->GetGPUDescriptorHandleForHeapStart());
        metallicTextureHandle.Offset(material.MetallicTextureIndex, mSrvDescriptorSize);
        ...
        cmdList->SetGraphicsRootDescriptorTable(4, roughnessTextureHandle);
        cmdList->SetGraphicsRootDescriptorTable(5, metallicTextureHandle);
        ...
        for (UINT objectIndex : mVisibleObjectIndices)
        {
            ...
            constants.MaterialParams = XMFLOAT4(material.Roughness, material.Metallic, material.AmbientOcclusion, 0.0f);
    ...
    {
        auto sceneToRenderTarget = CD3DX12_RESOURCE_BARRIER::Transition(
            mSceneColor.Get(), mSceneColorState, D3D12_RESOURCE_STATE_RENDER_TARGET);
        cmdList->ResourceBarrier(1, &sceneToRenderTarget);
    ...
    cmdList->ClearRenderTargetView(sceneColorRtv, clearColor, 0, nullptr);
    ...
    CD3DX12_GPU_DESCRIPTOR_HANDLE iblHandle(mLightingSrvHeap->GetGPUDescriptorHandleForHeapStart(), Gbuffer::TargetCount + 1, mSrvDescriptorSize);
    cmdList->SetGraphicsRootDescriptorTable(3, iblHandle);
    cmdList->IASetPrimitiveTopology(D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
    cmdList->IASetVertexBuffers(0, 0, nullptr);
    cmdList->IASetIndexBuffer(nullptr);
    ...
    {
        auto sceneToShaderResource = CD3DX12_RESOURCE_BARRIER::Transition(
            mSceneColor.Get(), mSceneColorState, D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE);
        cmdList->ResourceBarrier(1, &sceneToShaderResource);
        mSceneColorState = D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE;
    }
    ...
    UploadPostProcessConstants();
    cmdList->SetPipelineState(mPostProcessPSO.Get());
    cmdList->SetGraphicsRootSignature(mPostProcessRootSignature.Get());

    ID3D12DescriptorHeap* postHeaps[] = { mPostProcessSrvHeap.Get() };
    cmdList->SetDescriptorHeaps(1, postHeaps);
    cmdList->SetGraphicsRootConstantBufferView(0, mPostProcessConstantBuffer->GetGPUVirtualAddress());
    cmdList->SetGraphicsRootDescriptorTable(1, mPostProcessSrvHeap->GetGPUDescriptorHandleForHeapStart());
    cmdList->IASetPrimitiveTopology(D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
    cmdList->IASetVertexBuffers(0, 0, nullptr);
    cmdList->IASetIndexBuffer(nullptr);
    cmdList->DrawInstanced(3, 1, 0, 0);
